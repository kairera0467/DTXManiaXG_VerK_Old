...
//		private CTexture txレーンフレームGB;
				...
				if ( !pChip.bHit && pChip.b可視 )
                {
                    ...
                    switch (pChip.nチャンネル番号)
                    {
                        case 0x11:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(60 + 10, 128 + (num9 * 64), 0x2e + 10, 64));
                        ...
                        case 0x13:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(0, 128 + (num9 * 64), 60 + 10, 64));
                        ...
                        case 0x14:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(160 + 30, 128 + (num9 * 64), 0x2e + 10, 64));
                        ...
                        case 0x15:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(0xce + 40, 128 + (num9 * 64), 0x2e + 10, 64));
                        ...
                        case 0x16:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(298 + 60, 128 + (num9 * 64), 0x40 + 10, 64));
                        ...
                        case 0x17:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(0xfc + 50, 128 + (num9 * 64), 0x2e + 10, 64));
                            ...
                            if (this.txチップ != null)
                            {
                                switch (configIni.eHHOGraphics.Drums)
                                {
                                    case Eタイプ.A:
                                        ...
                                        this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(0x200 + 100, 128 + (num9 * 64), 0x26 + 10, 64));
                        ...
                        case 0x19:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(0x16a + 70, 128 + (num9 * 64), 0x26 + 10, 64));
                        ...
                        case 0x1a:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(448 + 90, 128 + (num9 * 64), 64 + 10, 64));
                        ...
                        case 0x1b:
                            ...
                            if (this.txチップ != null)
                            {
                                this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(550 + 110, 128 + (num9 * 64), 0x30 + 10, 64));
                        ...
                        case 0x1c:
                            ...
                            if (this.txチップ != null)
                            {
                                ...
                                {
                                    this.txチップ.t2D描画(CDTXMania.app.Device, x - 5, y - 32, new Rectangle(400 + 80, 128 + (num9 * 64), 0x30 + 10, 64));
        ...
        protected override void t進行描画・チップ・ボーナス(CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip)
        {
            ...
            if (!pChip.bHit && pChip.b可視)
            {
                ...
                int x = this.nボーナスチャンネルtoX座標[ pChip.n整数値 ];

                if (configIni.eLaneType.Drums == Eタイプ.A)
                {
                    if (configIni.eRDPosition == ERDPosition.RCRD)
                    {
                        x = this.nボーナスチャンネルtoX座標[pChip.n整数値];
                    }
                    else if (configIni.eRDPosition == ERDPosition.RDRC)
                    {
                ...
                else if (configIni.eLaneType.Drums == Eタイプ.B)
                {
                    if (configIni.eRDPosition == ERDPosition.RCRD)
                    {
                        x = this.nボーナスチャンネルtoX座標B[pChip.n整数値];
                    }
                    else if (configIni.eRDPosition == ERDPosition.RDRC)
                    {
                        ...
                        if (this.txチップ != null)
                        {
                            switch (pChip.n整数値)
                            {
                                case 0x01:
                                    ...
                                    break;
                                case 0x02:
                                    x = (x + 16) - ((int)((32.0 * pChip.dbチップサイズ倍率) / 2.0));
                                    if (this.txチップ != null)
                                    {
            ...
            if (!pChip.bHit && (pChip.nバーからの距離dot.Drums < 0))
            {
                ...
                if (((this.actCombo.n現在のコンボ数.Drums > 0) || configIni.bドラムが全部オートプレイである) && configIni.b歓声を発声する)
                {
                    ...
                    if (CDTXMania.ConfigIni.nSkillMode == 1 && ( !configIni.bドラムが全部オートプレイである || CDTXMania.ConfigIni.bAutoAddGage))
