...
namespace DTXMania
{
	internal class CAct演奏Guitarグラフ : CActivity
	{
        ...
        public double dbグラフ値現在_渡G
        {
            get
            {
                return this.dbグラフ値現在G;
            }
            set
            {
                this.dbグラフ値現在G = value;
            }
        }
        public double dbグラフ値目標_渡G
        {
            get
            {
                return this.dbグラフ値目標G;
            }
            set
            {
                this.dbグラフ値目標G = value;
            }
        }
        public double dbグラフ値現在_渡B
        {
            get
            {
                return this.dbグラフ値現在B;
            }
            set
            {
                this.dbグラフ値現在B = value;
            }
        }
        public double dbグラフ値目標_渡B
        {
            get
            {
                return this.dbグラフ値目標B;
            }
            set
            {
                this.dbグラフ値目標B = value;
            }
        }
		
		// コンストラクタ

		public CAct演奏Guitarグラフ()
		{
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
                ...
                double db1ノーツごとの達成率G = (double)this.dbグラフ値目標G / CDTXMania.DTX.n可視チップ数.Guitar;
                double db1ノーツごとの達成率B = (double)this.dbグラフ値目標B / CDTXMania.DTX.n可視チップ数.Bass;
                this.n現在演奏されたノーツ数G =
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Guitar.Perfect +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Guitar.Great +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Guitar.Good +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Guitar.Poor +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Guitar.Miss;
                this.n現在演奏されたノーツ数B =
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Bass.Perfect +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Bass.Great +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Bass.Good +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Bass.Poor +
                    CDTXMania.stage演奏ギター画面.nヒット数・Auto含む.Bass.Miss;
                CScoreIni.C演奏記録 guitar = new CScoreIni.C演奏記録();
                CScoreIni.C演奏記録 bass = new CScoreIni.C演奏記録();
                
                double rateG =  (double)n現在演奏されたノーツ数G / (double)CDTXMania.DTX.n可視チップ数.Guitar;
                double rateB =  (double)n現在演奏されたノーツ数B / (double)CDTXMania.DTX.n可視チップ数.Bass;
                ...
                if (CDTXMania.ConfigIni.nSkillMode == 0)
                {
                    ...
                    this.dbグラフ値比較G = this.dbグラフ値目標_渡G;
                    this.dbグラフ値比較B = this.dbグラフ値目標_渡B;
                }
                else if (CDTXMania.ConfigIni.nSkillMode == 1)
                {
                    //this.dbグラフ値比較 = CScoreIni.tゴーストスキルを計算して返す(CDTXMania.DTX.n可視チップ数.Drums, this.n現在演奏されたノーツ数, drums.n最大コンボ数, E楽器パート.DRUMS);
                    this.dbグラフ値比較G = (double)(db1ノーツごとの達成率G * n現在演奏されたノーツ数G);
                    this.dbグラフ値比較B = (double)(db1ノーツごとの達成率B * n現在演奏されたノーツ数B);
                }
                ...
                if (this.txグラフ != null)
                {
                    ...
                    {
                        this.txグラフバックパネル.t2D描画(CDTXMania.app.Device, 290, 0, rectangle);
                        this.txグラフバックパネル.t2D描画(CDTXMania.app.Device, 291, 650 - (int)(this.dbグラフ値現在G * 5.56), new Rectangle(499, 0, 201, (int)(this.dbグラフ値現在G * 5.56)));
                        this.t小文字表示(560, 658, string.Format("{0,6:##0.00}%", this.dbグラフ値現在G));
                        this.tx比較.t2D描画(CDTXMania.app.Device, 458, 200, new Rectangle(336, 0, 162, 60));
                        this.t小文字表示(542, 224, string.Format("{0,6:##0.00}%", this.dbグラフ値目標G));
                    }
                }
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    if (this.txグラフ != null)
                    {
                        this.txグラフ.t2D描画(CDTXMania.app.Device, 364, 94 + (int)(58.52 * i), rectangle);
                       
                    }
                }
                // グラフ
                // --現在値
                if (this.dbグラフ値現在_表示G < this.dbグラフ値現在G)
                {
                    this.dbグラフ値現在_表示G += (this.dbグラフ値現在G - this.dbグラフ値現在_表示G) / 5 + 0.01;
                }
                if (this.dbグラフ値現在_表示G >= this.dbグラフ値現在G)
                {
                    this.dbグラフ値現在_表示G = this.dbグラフ値現在G;
                }
                rectangle = new Rectangle(0, 0, 72, (int)(556f * this.dbグラフ値現在_表示G / 100));
                if (this.txグラフ != null)
                {
                ...
                // --現在値_目標越
                rectangle = new Rectangle(0, 0, 10, (int)(556f * this.dbグラフ値現在_表示G / 100));
                if ((dbグラフ値現在G >= dbグラフ値目標G) && (this.txグラフ != null))
                {
                ...
                if (this.dbグラフ値目標_表示G < this.dbグラフ値目標G)
                {
                    this.dbグラフ値目標_表示G += (this.dbグラフ値目標G - this.dbグラフ値目標_表示G) / 5 + 0.01;
                }
                if (this.dbグラフ値目標_表示G >= this.dbグラフ値目標G)
                {
                    this.dbグラフ値目標_表示G = this.dbグラフ値目標G;
                }
                ...
                //sw.Close();
                this.dbグラフ値目標_表示G = this.dbグラフ値目標G;
                rectangle = new Rectangle(138, 0, 72, (int)(556f * this.dbグラフ値目標_表示G / 100));
                if (this.txグラフ != null)
                {
                    //this.txグラフ.vc拡大縮小倍率 = new Vector3(1f, 1f, 1f);
                    //this.txグラフ.n透明度 = 192;
        ...
        public double db現在の判定数合計G;
        private double dbグラフ値目標G;
        public double dbグラフ値比較G;
        private double dbグラフ値目標_表示G;
        private double dbグラフ値現在G;
        private double dbグラフ値現在_表示G;
        private double dbグラフ値直前G;

        public double db現在の判定数合計B;
        private double dbグラフ値目標B;
        public double dbグラフ値比較B;
        private double dbグラフ値目標_表示B;
        private double dbグラフ値現在B;
        private double dbグラフ値現在_表示B;
        private double dbグラフ値直前B;

        private int n現在演奏されたノーツ数G;
        private int n現在演奏されたノーツ数B;
