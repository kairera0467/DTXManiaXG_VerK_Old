...
//			base.list子Activities.Add( this.actFI = new CActFIFOBlack() );	// #27787 2012.3.10 yyagi 曲読み込み画面のフェードインの省略
		...
		public override void On活性化()
		{
			...
			try
			{
                ...
                this.strアーティスト名 = "";
				this.strSTAGEFILE = "";
				this.ftタイトル表示用フォント = new Font( "MS PGothic", 36f, FontStyle.Regular, GraphicsUnit.Pixel );
                this.ftアーティスト名表示フォント = new Font("MS PGothic", 36f, FontStyle.Regular, GraphicsUnit.Pixel);
				this.nBGM再生開始時刻 = -1L;
				this.nBGMの総再生時間ms = 0;
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
                ...
                int nIndex = CDTXMania.nSongDifficulty;
                switch (nIndex)
                {
                    case 0: //DTX
                        難易度y = 0;
		...
		public override int On進行描画()
        {
            ...
            this.tx難易度パネル.t2D描画(CDTXMania.app.Device, 268, 194 , new Rectangle(130 , 難易度y, 130 , 72));
            ...
            string path = cdtx.strフォルダ名 + cdtx.PREIMAGE;
                ...
                if (this.txタイトル != null)
                {
                    ...
                    //this.txアーティスト.t2D描画(CDTXMania.app.Device, (int)(790 - (this.txアーティスト.szテクスチャサイズ.Width * this.txアーティスト.vc拡大縮小倍率.X)), 505);
                ...
                if (CDTXMania.ConfigIni.nRisky > 9)
                {
                    nCurrentRISKY = 0;
                }
