	...
	internal class CActResultParameterPanel : CActivity
	{
		...
		public override int On進行描画()
		{
			...
			for( int i = 0; i < 1; i++ )
			{
				...
				if( num >= 600 )
				{
                    this.t特大文字表示(x - 58, y + 173, CDTXMania.stage結果.st演奏記録[i].nスコア.ToString("0000000"), true);
			...
			if( this.ct表示用.n現在の値 >= 900 )
			{
				for( int j = 0; j < 3; j++ )
				{
                    ...
                    else if (CDTXMania.stage結果.st演奏記録[ j ].nスコア >= 1030000 )
                    {
                        if (this.ct表示用.b終了値に達した)
                        {
                            if (this.txExcellent != null)
                            {
                                this.txExcellent.t2D描画(CDTXMania.app.Device, this.n本体X + this.ptFullCombo位置[j].X, this.n本体Y + this.ptFullCombo位置[j].Y);
                            }
							if( !this.bフルコンボ音再生済み )
							{
								if( ( ( CDTXMania.DTX.SOUND_FULLCOMBO != null ) && ( CDTXMania.DTX.SOUND_FULLCOMBO.Length > 0 ) ) && File.Exists( CDTXMania.DTX.strフォルダ名 + CDTXMania.DTX.SOUND_FULLCOMBO ) )
