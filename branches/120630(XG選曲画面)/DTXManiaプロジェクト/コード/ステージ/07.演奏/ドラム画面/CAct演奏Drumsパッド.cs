	...
	internal class CAct演奏Drumsパッド : CActivity
	{
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
                ...
                {
                    this.txパッド = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\7_pads_XG3.png"));
                    this.tx光るパッド = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\ScreenPlayDrums pads flush_XG3.png" ) );
                }
                else
                {
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
            {
                ...
                while ((num - this.nフラッシュ制御タイマ) >= 15L)
                {
                    for (int j = 0; j < 10; j++)
                    {
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    int y = (this.st基本位置[index].y + (CDTXMania.ConfigIni.bReverse.Drums ? -10 : 560)) + this.stパッド状態[index].nY座標オフセットdot;
