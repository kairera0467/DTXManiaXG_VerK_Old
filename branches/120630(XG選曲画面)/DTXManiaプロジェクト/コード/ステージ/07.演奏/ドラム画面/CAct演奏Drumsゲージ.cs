...
//		}
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
				this.txゲージ = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\num.png" ) );
		...
		public override int On進行描画()
		{
			...
			if ( !base.b活性化してない )
			{
                ...
                {
                 {
                    base.txgbg.t2D描画(CDTXMania.app.Device, 0x102, 0x28f, new Rectangle(0, 0, 0x250, 0x2d));
                 }
                if (base.dbゲージ値 > 0.0)
                {
                    base.txgbar.vc拡大縮小倍率.X = (float)base.dbゲージ値;
                    base.txgbar.t2D描画(CDTXMania.app.Device, 0x12f, 0x299, new Rectangle(0, 0, 0x1f8, 0x1a));
                }
                base.txgbar.vc拡大縮小倍率.X = 1f;
                base.txgbar.t2D描画(CDTXMania.app.Device, 0x133, 0x299, new Rectangle(0, 0x1a, 0x1f8, 0x1a));
                 }
                ...
                {
                    this.txゲージマスク2.t2D描画(CDTXMania.app.Device, 259, 652);
                }
                if (base.db現在のゲージ値.Drums == 1.0)
                {
                    this.txゲージマスク.t2D描画(CDTXMania.app.Device, 259, 652);
                }
			}
			return 0;
		}
