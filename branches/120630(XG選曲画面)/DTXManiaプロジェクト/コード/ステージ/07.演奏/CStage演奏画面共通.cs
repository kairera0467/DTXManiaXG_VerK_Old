...
//			GCSettings.LatencyMode = GCLatencyMode.LowLatency;	// GCのgen2はせいぜい1演奏辺り1,2回しか発生しないので、ここでわざわざGC gen2は抑止しない
        ...
        static CStage演奏画面共通()
        {
            nJudgeLineMinPosY = 260;
            nJudgeLineMaxPosY = 561;
            nShutterInMinPosY = 0;
            nShutterInMaxPosY = 720;
        }
        ...
        protected static int nShutterInMaxPosY;
        protected static int nShutterInMinPosY;
...
//		protected GCLatencyMode gclatencymode;
		...
		protected E判定 tチップのヒット処理( long nHitTime, CDTX.CChip pChip, E楽器パート screenmode, bool bCorrectLane )
		{
            ...
            else if (CDTXMania.ConfigIni.bAutoAddGage == false)
            {
                    if ( (pChip.e楽器パート != E楽器パート.UNKNOWN))
                    {
                        actGauge.Damage(screenmode, pChip.e楽器パート, eJudgeResult);
            ...
            if (CDTXMania.ConfigIni.bAutoAddGage)
            {
				...
				else if ( ( eJudgeResult == E判定.Perfect ) )
				{
                    ...
                    {
                        nScoreDelta = 1000000 / 50 / (float)(nComboMax - 24.5);
                    }
                    else if (eJudgeResult == E判定.Great)
                    {
                        nScoreDelta = 1000000 / 50 / (float)(nComboMax - 24.5) * (float)0.5;
                    }
                    else if (eJudgeResult == E判定.Good)
                    {
                        nScoreDelta = 1000000 / 50 / (float)(nComboMax - 24.5) * (float)0.2;
                    }
...
//					nTimeDiff = Math.Abs( (int) ( nTime - nearestChip.n発声時刻ms ) );
		...
		protected void tキー入力()
		{
			...
			if ( (( base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED ) ) && ( base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) )
			{
                ...
                if (keyboard.bキーが押されている( (int) SlimDX.DirectInput.Key.PageUp))
                {
                    ...
                    CDTXMania.ConfigIni.nJudgeLine = (nJudgeLineMaxPosY - this.nJudgeLinePosY)-620;
                ...
                if (keyboard.bキーが押されている((int)SlimDX.DirectInput.Key.NumberPad8))
                {
                    ...
                    CDTXMania.ConfigIni.nShutterInSide = nShutterInMaxPosY - this.nShutterInPosY -620;
                }
                ...
                if (keyboard.bキーが押されている((int)SlimDX.DirectInput.Key.NumberPad4))
                {
                    ...
                    CDTXMania.ConfigIni.nShutterOutSide = nShutterInMaxPosY - this.nShutterOutPosY - 720;
                }
                if (keyboard.bキーが押されている((int)SlimDX.DirectInput.Key.NumberPad6))
                {
                    ...
                    CDTXMania.ConfigIni.nShutterOutSide = nShutterInMaxPosY - this.nShutterOutPosY - 620;
                }
        ...
        protected void t進行描画・STAGECLEARED()
        {
            if (CDTXMania.stage演奏ドラム画面.ct進行.b終了値に達した)
            {
				...
				switch ( pChip.nチャンネル番号 )
				{
					...
					case 0x03:	// BPM変更
						if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
						{
                            ...
                            CDTXMania.stage演奏ドラム画面.UnitTime = (int)((60 / (CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM) / 8 * 600));
...
//Trace.TraceInformation( "chip={0:x2}, E楽器パート={1}, x={2}", pChip.nチャンネル番号, inst, x );
		...
		protected void t進行描画・判定ライン()
		{
			if ( CDTXMania.ConfigIni.eDark != Eダークモード.FULL )
			{
                int y = CDTXMania.ConfigIni.bReverse.Drums ? 0x35 : nJudgeLinePosY;
                this.txヒットバー.t2D描画(CDTXMania.app.Device, 0x127, y, new Rectangle(0, 0, 0x22f, 6));
			}
		}
