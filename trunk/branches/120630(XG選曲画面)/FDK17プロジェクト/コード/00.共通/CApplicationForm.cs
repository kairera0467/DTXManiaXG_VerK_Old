...
namespace FDK
{
	public class CApplicationForm : IDisposable
	{
		...
		public void Run()
		{
			...
			C変換.tDisposeする( this.D3D9Device );
			//-----------------
			#endregion
			#region [ Direct3D9 を解放。]
			//-----------------
			C変換.tDisposeする( this.Direct3D );
			//-----------------
			#endregion
		}
		...
		public void Dispose()
		{
			C変換.tDisposeする( this.D3D9Device ); this.D3D9Device = null;
			C変換.tDisposeする( this.Direct3D ); this.Direct3D = null;
		}
		//-----------------
		#endregion
		...
		protected const uint wsウィンドウスタイル =
			CWin32.WS_OVERLAPPED |		// オーバラップウィンドウ。
			CWin32.WS_BORDER |			// 境界を持つ。
			CWin32.WS_CAPTION |			// タイトルバーを持つ。（WS_DLGFRAME スタイルと一緒に使うことは不可。）
			CWin32.WS_THICKFRAME |		// ウィンドウのサイズ変更に使うことができる太い枠を持つ。
			CWin32.WS_CLIPCHILDREN |	// 親ウィンドウの内部で描画するときに、子ウィンドウが占める領域を除外する。親ウィンドウを作成するときに使う。
			CWin32.WS_SYSMENU |			// タイトルバーにコントロールメニューボックスと「閉じる」ボタンを持つ。
			CWin32.WS_EX_APPWINDOW;		// ウィンドウが表示されているときには、必ずトップレベルウィンドウがタスクバー上に置かれる。
		protected const uint ws全画面スタイル =
			CWin32.WS_POPUP |			// ポップアップウィンドウ。（タイトルバー、枠なし。クライアント領域のみ。）
			CWin32.WS_VISIBLE;			// 初期状態で可視。
		//-----------------
		#endregion
