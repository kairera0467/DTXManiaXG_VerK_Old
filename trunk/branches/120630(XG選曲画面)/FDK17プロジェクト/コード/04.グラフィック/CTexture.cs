 ...
 this.sz画像サイズ.Width;
        ...
        public void t2D描画(Device device, int x, int y, float depth, Rectangle rc画像内の描画領域)
        {
            ...
            else
            {
                ...
                if (this.fZ軸中心回転 == 0f)
                {
                    float num5 = (rc画像内の描画領域.Width % 2 == 0) ? -0.5f : 0f;
                    float num6 = (rc画像内の描画領域.Height % 2 == 0) ? -0.5f : 0f;
                    float num7 = (float)rc画像内の描画領域.Width / 2f;
                    float num8 = (float)rc画像内の描画領域.Height / 2f;
                    int width = rc画像内の描画領域.Width;
                    int height = rc画像内の描画領域.Height;
                    float x4 = (float)rc画像内の描画領域.Left / (float)this.szテクスチャサイズ.Width;
                    float x5 = (float)rc画像内の描画領域.Right / (float)this.szテクスチャサイズ.Width;
                    float y4 = (float)rc画像内の描画領域.Top / (float)this.szテクスチャサイズ.Height;
                    float y5 = (float)rc画像内の描画領域.Bottom / (float)this.szテクスチャサイズ.Height;
                    int color2 = new Color4((float)this._透明度 / 255f, 1f, 1f, 1f).ToArgb();
                    if (this.cvPositionColoredVertexies == null)
                    {
                        this.cvPositionColoredVertexies = new PositionColoredTexturedVertex[4];
                    }
                    this.cvPositionColoredVertexies[0].Position = new Vector3(0f, num8 + num6, depth);
                    this.cvPositionColoredVertexies[0].Color = color2;
                    this.cvPositionColoredVertexies[0].TextureCoordinates = new Vector2(x4, y4);
                    this.cvPositionColoredVertexies[1].Position = new Vector3((float)rc画像内の描画領域.Width, num8 + num6, depth);
                    this.cvPositionColoredVertexies[1].Color = color2;
                    this.cvPositionColoredVertexies[1].TextureCoordinates = new Vector2(x5, y4);
                    this.cvPositionColoredVertexies[2].Position = new Vector3(0f, -num8 + num6, depth);
                    this.cvPositionColoredVertexies[2].Color = color2;
                    this.cvPositionColoredVertexies[2].TextureCoordinates = new Vector2(x4, y5);
                    this.cvPositionColoredVertexies[3].Position = new Vector3((float)rc画像内の描画領域.Width + num5, -num8 + num6, depth);
                    this.cvPositionColoredVertexies[3].Color = color2;
                    this.cvPositionColoredVertexies[3].TextureCoordinates = new Vector2(x5, y5);
                    using (DataStream dataStream = this.vbPositionColoredVertexBuffer.Lock(0, 0, LockFlags.None))
                    {
                        dataStream.WriteRange<PositionColoredTexturedVertex>(this.cvPositionColoredVertexies);
                        this.vbPositionColoredVertexBuffer.Unlock();
                    }
                    int num9 = x + rc画像内の描画領域.Width / 2;
                    int num10 = y + rc画像内の描画領域.Height / 2;
                    Vector3 amount = new Vector3((float)num9 - (float)device.Viewport.Width / 2f, -((float)num10 - (float)device.Viewport.Height / 2f), 0f);
                    Matrix matrix = Matrix.Identity * Matrix.RotationY(this.fY軸中心回転);
                    matrix *= Matrix.Scaling(this.vc拡大縮小倍率);
        ...
        public void tDrawScore(Device device, int x, int y, Rectangle rc画像内の描画領域, int digitno)
        {
            this.tDrawScore(device, x, y, 1f, rc画像内の描画領域, digitno);
        }
        public void tDrawScore(Device device, int x, int y, float depth, Rectangle rc画像内の描画領域, int digitno)
        {
            float f補正値X = (rc画像内の描画領域.Width % 2 == 0) ? -0.5f : 0f;
            float f補正値Y = (rc画像内の描画領域.Height % 2 == 0) ? -0.5f : 0f;
            float f中央X = (float)rc画像内の描画領域.Width / 2f;
            float f中央Y = (float)rc画像内の描画領域.Height / 2f;
            int width = rc画像内の描画領域.Width;
            int height = rc画像内の描画領域.Height;
            float f左U値 = (float)rc画像内の描画領域.Left / (float)this.szテクスチャサイズ.Width;
            float f右U値 = (float)rc画像内の描画領域.Right / (float)this.szテクスチャサイズ.Width;
            float y2 = (float)rc画像内の描画領域.Top / (float)this.szテクスチャサイズ.Height;
            float y3 = (float)rc画像内の描画領域.Bottom / (float)this.szテクスチャサイズ.Height;
            int color = new Color4((float)this._透明度 / 255f, 1f, 1f, 1f).ToArgb();
            if (this.cvPositionColoredVertexies == null)
            {
                this.cvPositionColoredVertexies = new PositionColoredTexturedVertex[4];
            }
            this.cvPositionColoredVertexies[0].Position = new Vector3(0f + (float)(digitno * 13), f中央Y + f補正値Y, depth);
            this.cvPositionColoredVertexies[0].Color = color;
            this.cvPositionColoredVertexies[0].TextureCoordinates = new Vector2(f左U値, y2);
            this.cvPositionColoredVertexies[1].Position = new Vector3((float)rc画像内の描画領域.Width + (float)(digitno * 13), f中央Y + f補正値Y, depth);
            this.cvPositionColoredVertexies[1].Color = color;
            this.cvPositionColoredVertexies[1].TextureCoordinates = new Vector2(f右U値, y2);
            this.cvPositionColoredVertexies[2].Position = new Vector3(0f + (float)(digitno * 13), -f中央Y + f補正値Y, depth);
            this.cvPositionColoredVertexies[2].Color = color;
            this.cvPositionColoredVertexies[2].TextureCoordinates = new Vector2(f左U値, y3);
            this.cvPositionColoredVertexies[3].Position = new Vector3((float)rc画像内の描画領域.Width + f補正値X + (float)(digitno * 13), -f中央Y + f補正値Y, depth);
            this.cvPositionColoredVertexies[3].Color = color;
            this.cvPositionColoredVertexies[3].TextureCoordinates = new Vector2(f右U値, y3);
            using (DataStream dataStream = this.vbPositionColoredVertexBuffer.Lock(0, 0, LockFlags.None))
            {
                dataStream.WriteRange<PositionColoredTexturedVertex>(this.cvPositionColoredVertexies);
                this.vbPositionColoredVertexBuffer.Unlock();
            }
            int num5 = x + rc画像内の描画領域.Width / 2;
            int num6 = y + rc画像内の描画領域.Height / 2;
            Vector3 amount = new Vector3((float)num5 - (float)device.Viewport.Width / 2f, -((float)num6 - (float)device.Viewport.Height / 2f) - (float)digitno / 1.2f, 0f);
            Matrix matrix = Matrix.Identity * Matrix.RotationY(this.fY軸中心回転);
            matrix *= Matrix.Scaling(this.vc拡大縮小倍率);
            matrix *= Matrix.Translation(amount);
            device.SetTransform(TransformState.World, matrix);
