...
namespace FDK
{
	...
	public class CDirectShow : IDisposable
	{
		...
		public CDirectShow( string fileName, IntPtr hWnd, bool bオーディオレンダラなし )
		{
			...
			try
			{
...

				{
					...
					{
						C変換.tCOMオブジェクトを解放する( ref videoInputPin );
						C変換.tCOMオブジェクトを解放する( ref videoRenderer );
					}
				}
				...
				while( eFilters.Next( 1, filters, IntPtr.Zero ) == CWin32.S_OK )
				{
					...
					DsError.ThrowExceptionForHR( hr );
					{
						...
						if( filterInfo.pGraph != null )
							C変換.tCOMオブジェクトを解放する( ref filterInfo.pGraph );
					}
									...
									DsError.ThrowExceptionForHR( hr );
									{
										...
										DsError.ThrowExceptionForHR( hr );
										{
											...
											if( connectFilterInfo.pGraph != null )
												C変換.tCOMオブジェクトを解放する( ref connectFilterInfo.pGraph );
										}

										Debug.WriteLine( connectPinInfo.name );		// 接続先ピン名
										if( connectPinInfo.filter != null )
											C変換.tCOMオブジェクトを解放する( ref connectPinInfo.filter );
										DsUtils.FreePinInfo( connectPinInfo );
									}
									C変換.tCOMオブジェクトを解放する( ref connectPin );
								}
								if( pinInfo.filter != null )
									C変換.tCOMオブジェクトを解放する( ref pinInfo.filter );
								DsUtils.FreePinInfo( pinInfo );
							}
							C変換.tCOMオブジェクトを解放する( ref pins[ 0 ] );
						}
					}
					C変換.tCOMオブジェクトを解放する( ref ePins );

					C変換.tCOMオブジェクトを解放する( ref filters[ 0 ] );
				}
			}
			C変換.tCOMオブジェクトを解放する( ref eFilters );
		...
		public static void tオーディオレンダラをNullレンダラに変えてフォーマットを取得する( IGraphBuilder graphBuilder, out WaveFormat wfx, out byte[] wfx拡張データ )
		{
			...
			{
				C変換.tCOMオブジェクトを解放する( ref nullRendererInputPin );
				C変換.tCOMオブジェクトを解放する( ref nullRenderer );
				C変換.tCOMオブジェクトを解放する( ref rendererConnectedOutputPin );
				C変換.tCOMオブジェクトを解放する( ref rendererInputPin );
				C変換.tCOMオブジェクトを解放する( ref audioRenderer );
			}
		}
		public static void tビデオレンダラをグラフから除去する( IGraphBuilder graphBuilder )
		{
			...
			{
				C変換.tCOMオブジェクトを解放する( ref connectedOutputPin );
				C変換.tCOMオブジェクトを解放する( ref renderInputPin );
				C変換.tCOMオブジェクトを解放する( ref videoRenderer );
			}
		}
				...
				while( ePins.Next( 1, pins, IntPtr.Zero ) == CWin32.S_OK )
				{
					...
					finally
					{
						if( pinfo.filter != null )
							C変換.tCOMオブジェクトを解放する( ref pinfo.filter );
		...
		protected virtual void Dispose( bool bManagedリソースも解放する )
		{
			...
			C変換.tCOMオブジェクトを解放する( ref this.nullRenderer );
			C変換.tCOMオブジェクトを解放する( ref this.memoryRenderer );
			C変換.tCOMオブジェクトを解放する( ref this.memoryRendererObject );
			C変換.tCOMオブジェクトを解放する( ref this.graphBuilder );
			//-----------------
			#endregion

			C変換.t完全なガベージコレクションを実施する();
		}
        ~CDirectShow()
		{
