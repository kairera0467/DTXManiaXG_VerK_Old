...
namespace DTXMania
{
	internal class CDTXMania : Game
	{
		...
		public static readonly string VERSION = "Ver1.70β(12????)";
        ...
        {
            CDTXMania.App = app;
            CDTXMania.Folder = new Folder();                           // ユーザ個別フォルダのみユーザ依存
            CDTXMania.app.bウィンドウがアクティブである = false;
...
//			new GCBeep();
		...
		protected override void LoadContent()
		{
            ...
            if (ConfigIni.クレジットを表示する == true)
            {
                ...
                int width = (int)graphics.MeasureString("DTXManiaXG(verK) ver 1.61     based on DTXMania", this.ftDeclaration).Width;
                graphics.Dispose();
                Bitmap bitmap2 = new Bitmap(width, this.ftDeclaration.Height);
                graphics = Graphics.FromImage(bitmap2);
                graphics.DrawString("DTXManiaXG(verK) ver 1.61     based on DTXMania", this.ftDeclaration, Brushes.White, (float)0f, (float)0f);
                graphics.Dispose();
...
// Debug.WriteLine( "capture: " + string.Format( "{0:2x}", (int) e.KeyCode ) + " " + (int) e.KeyCode );
        ...
        public static void t現在のユーザConfigに従ってサウンドデバイスとすべての既存サウンドを再構築する()
        {
            ...
            if (CDTXMania.SoundDevice != null)
            {
                ...
                C共通.tDisposeする(CDTXMania.SoundDevice); CDTXMania.SoundDevice = null;
                C共通.tDisposeする(ref CDTXMania.rc演奏用タイマ);	// Global.SoundDevice を解放した後に解放すること。（Global.SoundDevice で参照されているため）
            }
            //-----------------
            #endregion
            ...
            //-----------------
            switch (CDTXMania.ConfigIni.SoundDeviceType)
            {
                //case ESoundDeviceType.ExclusiveWASAPI:
                //    CDTXMania.SoundDevice = new CSoundDeviceWASAPI(CSoundDeviceWASAPI.Eデバイスモード.排他, Global.Users.Config.SoundDelayExclusiveWASAPI, Global.Users.Config.SoundUpdatePeriodExclusiveWASAPI);
                //    break;
            
                //case ESoundDeviceType.SharedWASAPI:
                //    CDTXMania.SoundDevice = new CSoundDeviceWASAPI(CSoundDeviceWASAPI.Eデバイスモード.共有, Global.Users.Config.SoundDelaySharedWASAPI, Global.Users.Config.SoundUpdatePeriodSharedWASAPI);
                //    break;
                ...
                case ESoundDeviceType.DirectSound:
                    CDTXMania.SoundDevice = new CSoundDeviceDirectSound(Global.App.Window.Handle, CDTXMania.ConfigIni.SoundDelayDirectSound);
                    break;

                default:
                    throw new Exception(string.Format("未対応の SoundDeviceType です。[{0}]", CDTXMania.ConfigIni.SoundDeviceType.ToString()));
            }
            //-----------------
            #endregion
            #region [ 新しい演奏タイマを構築する。]
            //-----------------
            Global.rc演奏用タイマ = new CSoundTimer(CDTXMania.SoundDevice);
            //-----------------
            #endregion

            CSound.tすべてのサウンドを再構築する(CDTXMania.SoundDevice);		// すでに生成済みのサウンドがあれば作り直す。
#endif
        }
