...
namespace DTXMania
{
	internal class CAct演奏Combo共通 : CActivity
	{
		...
		protected virtual void tコンボ表示・ドラム( int nCombo値, int nジャンプインデックス )
		{
            ...
            nUnitTime = (float) (60 / CDTXMania.DTX.BPM ) / 4;
			...
			if( this.txCOMBOドラム != null )
                if (e表示位置 == Eドラムコンボ文字の表示位置.RIGHT)
                {
                    if (n桁数 == 2)//3ケタ未満の場合
                    {
                        this.txCOMBOドラム.t2D描画(CDTXMania.app.Device, x - 162, y + 36, new Rectangle(0, 230, 160, 64));	// "COMBO" を表示。
                    ...
                    {
                        this.txCOMBOドラム.t2D描画(CDTXMania.app.Device, x - 315, y + 36, new Rectangle(0, 357, 318, 63));
                    }
                }
                else
                {
                    if (n桁数 == 2)
                    {
                        this.txCOMBOドラム.t2D描画(CDTXMania.app.Device, x - 94, y + 50, new Rectangle(0, 140, 90, 32));	// "COMBO" を表示。
                    }
                    else if (n桁数 == 3)
                    {
		...
		public override void OnManagedリソースの作成()
		{
            ...
            {
                this.txCOMBOドラム = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\ScreenPlayDrums combo drums_XG3.png"));
            }
            else
            {
