...
namespace DTXMania
{
	internal class CAct演奏パネル文字列 : CActivity
	{
		...
		{
            this.ft表示用フォント = new Font("ＤＦＧ平成ゴシック体W5", 48f, FontStyle.Bold, GraphicsUnit.Pixel);
			this.n文字列の長さdot = 0;
			this.txPanel = null;
        ...
        public int t進行描画(int x, int y)
		{
			if( !base.b活性化してない )
			{
				...
				Rectangle rectangle = new Rectangle( (int) ( ( (float) this.ct進行用.n現在の値 ) / num ), 0, (int) ( 360f / num ), (int) this.ft表示用フォント.Size );
				if( rectangle.X < 0 )
				{
