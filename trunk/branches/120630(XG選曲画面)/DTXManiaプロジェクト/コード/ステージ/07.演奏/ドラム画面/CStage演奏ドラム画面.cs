...
namespace DTXMania
{
	internal class CStage演奏ドラム画面 : CStage演奏画面共通
	{
		...
		public override void On活性化()
		{
            ...
            base.nJudgeLinePosY += CDTXMania.ConfigIni.nJudgeLine;
            this.actChipFireD.iPosY = base.nJudgeLinePosY - 186;
            base.actPlayInfo.jl = CStage演奏画面共通.nJudgeLineMaxPosY - base.nJudgeLinePosY;
            base.nShutterInPosY += CDTXMania.ConfigIni.nShutterInSide;
            base.nShutterOutPosY += CDTXMania.ConfigIni.nShutterOutSide;
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
                ...
                if ((CDTXMania.DTX.bチップがある.LeftCymbal == false) && (CDTXMania.DTX.bチップがある.LP == false) && (CDTXMania.DTX.bチップがある.LBD == false) && (CDTXMania.DTX.bチップがある.FT == false) && (CDTXMania.DTX.bチップがある.Ride == false))
                {
                    ...
                    if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.C)
                    {
                        if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RCRD)
                        {
                            this.txLaneCover = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\7_lanes_Cover_cls.png"));
		...
		public override int On進行描画()
		{
			...
			if( !base.b活性化してない )
            {
                ...
                if (base.b初めての進行描画)
                {
                    ...
                    UnitTime = (int)((60 / (CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM) / 8 * 600));
#if 封印
                    while (bIsFinishedPlaying == false)
                    {
                        if (UnitTime != CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM)
                        {
                            UnitTime = (int)((60 / (CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM) / 8 * 600));

                            System.IO.StreamWriter sw = new System.IO.StreamWriter(@"debug.txt", true, System.Text.Encoding.GetEncoding("shift_jis"));
                            sw.WriteLine("{0}小節・1拍目", CDTXMania.stage演奏ドラム画面.actPlayInfo.n小節番号);
                            sw.WriteLine("現在のUnitTimeは{0}で、BPMは{1}です。", UnitTime, CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM);
                            sw.Close();
                        }
                        else
                        {
                            if (bIsFinishedPlaying == true)
                            {
                                break;
                            }
                        }
                    ...
                    this.ct進行 = new CCounter(0, 5, 100, CDTXMania.Timer);
                    ...
                    //CDTXMania.stage演奏ドラム画面.actFillin.ct爆発.n現在の値 = 0;
                ...
                int num = -720 + (int)(base.nShutterInPosY * 7.2f);
                this.txシャッター.t2D描画(CDTXMania.app.Device, 295, num);
                ...
                {
                    this.ct進行.t進行();
                    int Full = base.nヒット数・Auto含まない.Drums.Miss + base.nヒット数・Auto含まない.Drums.Poor;
                    switch (Full)
                    {
                        case 0:
                            {
                                int Bonus = base.nヒット数・Auto含まない.Drums.Perfect;
                                if (CDTXMania.ConfigIni.bドラムが全部オートプレイである)
                                {
                                    Bonus = base.nヒット数・Auto含む.Drums.Perfect;
                                }
                                    if(Bonus == CDTXMania.DTX.n可視チップ数.Drums)
                                        #region[ エクセ ]
                                        { 
                                            this.actScore.n現在の本当のスコア.Drums += (1000000 - this.actScore.n現在の本当のスコア.Drums) + 30000;
        ...
        private readonly int[] nチャンネルtoX座標 = new int[] { 370, 470, 582, 527, 645, 748, 694, 373, 815, 298, 419, 419 };
        private readonly int[] nチャンネルtoX座標B = new int[] { 371, 420, 534, 597, 645, 748, 695, 373, 815, 298, 477, 477 };
        private readonly int[] nチャンネルtoX座標C = new int[] { 370, 470, 534, 597, 645, 748, 694, 373, 815, 298, 419, 419 };
        private readonly int[] nチャンネルtoX座標D = new int[] { 370, 470, 582, 527, 645, 748, 694, 373, 815, 298, 419, 419 };
        private readonly int[] nチャンネルtoX座標改 = new int[] { 370, 470, 582, 527, 645, 786, 694, 373, 748, 298, 419, 419 };
        private readonly int[] nチャンネルtoX座標B改 = new int[] { 371, 420, 534, 597, 645, 786, 695, 373, 748, 298, 477, 477 };
        private readonly int[] nチャンネルtoX座標C改 = new int[] { 370, 470, 534, 597, 645, 786, 694, 373, 748, 298, 419, 419 };
        private readonly int[] nチャンネルtoX座標D改 = new int[] { 370, 470, 582, 527, 645, 786, 694, 373, 748, 298, 419, 419 };
        ...
        {
            this.actJudgeString.iP_A = base.nJudgeLinePosY - 0xbd;
            this.actJudgeString.iP_B = base.nJudgeLinePosY + 0x17;
            this.actChipFireD.iPosY = base.nJudgeLinePosY - 0xba;
            CDTXMania.stage演奏ドラム画面.actPlayInfo.jl = CStage演奏画面共通.nJudgeLineMaxPosY - base.nJudgeLinePosY;
        }
...
//#if 封印
                        ...
                        #region [ LPのヒット処理 ]
                            ...
                            {
                                        chip28 = base.r指定時刻に一番近い未ヒットChip(nTime, 0x1b, nInputAdjustTime);
                                        chip29 = base.r指定時刻に一番近い未ヒットChip(nTime, 0x1c, nInputAdjustTime);
                                        chip30 = base.r指定時刻に一番近い未ヒットChip(nTime, 0x13, nInputAdjustTime);
                                        e判定25 = (chip28 != null) ? base.e指定時刻からChipのJUDGEを返す(nTime, chip28, nInputAdjustTime) : E判定.Miss;
                                        e判定26 = (chip29 != null) ? base.e指定時刻からChipのJUDGEを返す(nTime, chip29, nInputAdjustTime) : E判定.Miss;
                                        e判定27 = (chip30 != null) ? base.e指定時刻からChipのJUDGEを返す(nTime, chip30, nInputAdjustTime) : E判定.Miss;
                                        E判定[] e判定Array = new E判定[] { e判定LP, e判定LBD, e判定BD };
                                        ...
                                        switch (eBDGroup)
                                        {
                                            ...
                                            case EBDGroup.左右ペダルのみ打ち分ける:
                                                #region[LP&LBD | BD]
                                                if ((e判定25 != E判定.Miss) && (e判定26 != E判定.Miss))
                                                {
                                                    if (chip28.n発声位置 < chip29.n発声位置)
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip28, inputEvent.nVelocity);
                                                    }
                                                    else if (chip28.n発声位置 > chip29.n発声位置)
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip29, inputEvent.nVelocity);
                                                    }
                                                    else
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip28, inputEvent.nVelocity);
                                                ...
                                                else if (e判定25 != E判定.Miss)
                                                {
                                                    this.tドラムヒット処理(nTime, Eパッド.LP, chip28, inputEvent.nVelocity);
                                                    bHitted = true;
                                                }
                                                else if (e判定26 != E判定.Miss)
                                                {
                                                    this.tドラムヒット処理(nTime, Eパッド.LP, chip29, inputEvent.nVelocity);
                                                    bHitted = true;
                                                }
                                            ...
                                            case EBDGroup.どっちもBD:
                                                #region[LP&LBD&BD]
                                                    if (((e判定25 != E判定.Miss) && (e判定26 != E判定.Miss)) && (e判定27 != E判定.Miss))
                                                {
                                                    CDTX.CChip chip31;
                                                    CDTX.CChip[] chipArray7 = new CDTX.CChip[] { chip28, chip29, chip30 };
                                                ...
                                                else if ((e判定25 != E判定.Miss) && (e判定26 != E判定.Miss))
                                                {
                                                    if (chip28.n発声位置 < chip29.n発声位置)
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip28, inputEvent.nVelocity);
                                                ...
                                                else if ((e判定25 != E判定.Miss) && (e判定27 != E判定.Miss))
                                                {
                                                    if (chip28.n発声位置 < chip30.n発声位置)
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip28, inputEvent.nVelocity);
                                                    }
                                                    else if (chip28.n発声位置 > chip30.n発声位置)
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip30, inputEvent.nVelocity);
                                                    }
                                                    else
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip28, inputEvent.nVelocity);
                                                ...
                                                else if ((e判定26 != E判定.Miss) && (e判定27 != E判定.Miss))
                                                {
                                                    if (chip29.n発声位置 < chip30.n発声位置)
                                                    {
                                                        this.tドラムヒット処理(nTime, Eパッド.LP, chip29, inputEvent.nVelocity);
                                                    }
                        ...
                        #region [ LBDのヒット処理 ]
                            ...
                            chip32 = base.r指定時刻に一番近い未ヒットChip(nTime, 0x1b, nInputAdjustTime);
                            chip33 = base.r指定時刻に一番近い未ヒットChip(nTime, 0x1c, nInputAdjustTime);
                            chip34 = base.r指定時刻に一番近い未ヒットChip(nTime, 0x13, nInputAdjustTime);
                            e判定28 = (chip32 != null) ? base.e指定時刻からChipのJUDGEを返す(nTime, chip32, nInputAdjustTime) : E判定.Miss;
                            e判定29 = (chip33 != null) ? base.e指定時刻からChipのJUDGEを返す(nTime, chip33, nInputAdjustTime) : E判定.Miss;
                            e判定30 = (chip34 != null) ? base.e指定時刻からChipのJUDGEを返す(nTime, chip34, nInputAdjustTime) : E判定.Miss;
                            switch (eBDGroup)
                            {
                                case EBDGroup.打ち分ける:
                                    if (e判定29 != E判定.Miss)
                                    {
                                        this.tドラムヒット処理(nTime, Eパッド.LBD, chip33, inputEvent.nVelocity);
                                        bHitted = true;
                                    }
                                    if (bHitted)
                                    {
                                ...
                                case EBDGroup.左右ペダルのみ打ち分ける:
                                    if ((e判定28 != E判定.Miss) && (e判定29 != E判定.Miss))
                                    {
                                        if (chip32.n発声位置 < chip33.n発声位置)
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip32, inputEvent.nVelocity);
                                        }
                                        else if (chip32.n発声位置 > chip33.n発声位置)
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip33, inputEvent.nVelocity);
                                        }
                                        else
                                    ...
                                    else if (e判定28 != E判定.Miss)
                                    {
                                        this.tドラムヒット処理(nTime, Eパッド.LBD, chip32, inputEvent.nVelocity);
                                        bHitted = true;
                                    }
                                    else if (e判定29 != E判定.Miss)
                                    {
                                        this.tドラムヒット処理(nTime, Eパッド.LBD, chip33, inputEvent.nVelocity);
                                        bHitted= true;
                                    }
                                ...
                                case EBDGroup.どっちもBD:
                                    if (((e判定28 != E判定.Miss) && (e判定29 != E判定.Miss)) && (e判定30 != E判定.Miss))
                                    {
                                        CDTX.CChip chip35;
                                        CDTX.CChip[] chipArray8 = new CDTX.CChip[] { chip32, chip33, chip34 };
                                    ...
                                    else if ((e判定28 != E判定.Miss) && (e判定29 != E判定.Miss))
                                    {
                                        if (chip32.n発声位置 < chip33.n発声位置)
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip32, inputEvent.nVelocity);
                                    ...
                                    else if ((e判定28 != E判定.Miss) && (e判定30 != E判定.Miss))
                                    {
                                        if (chip32.n発声位置 < chip34.n発声位置)
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip32, inputEvent.nVelocity);
                                        }
                                        else if (chip32.n発声位置 > chip34.n発声位置)
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip34, inputEvent.nVelocity);
                                        }
                                        else
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip32, inputEvent.nVelocity);
                                    ...
                                    else if ((e判定29 != E判定.Miss) && (e判定30 != E判定.Miss))
                                    {
                                        if (chip33.n発声位置 < chip34.n発声位置)
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip33, inputEvent.nVelocity);
                                        }
                                        else if (chip33.n発声位置 > chip34.n発声位置)
                                        {
                                            this.tドラムヒット処理(nTime, Eパッド.LBD, chip34, inputEvent.nVelocity);
                                        }
                                        else
		...
		protected override void t進行描画・チップ・ドラムス( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			if ( configIni.bDrums有効 )
			{
				...
				if ( !pChip.bHit && pChip.b可視 )
                {
                    ...
                    int y = configIni.bReverse.Drums ? (0x38 + pChip.nバーからの距離dot.Drums) : (base.nJudgeLinePosY - pChip.nバーからの距離dot.Drums);
                    if (base.txチップ != null)
                    {
			...
			if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
			{
				...
				switch ( pChip.n整数値 )
				{
					...
					case 0x02:	// フィルイン終了
                        ...
                        this.actStatusPanels.nCurrentScore += 500;
						}
						break;
				}
			}
