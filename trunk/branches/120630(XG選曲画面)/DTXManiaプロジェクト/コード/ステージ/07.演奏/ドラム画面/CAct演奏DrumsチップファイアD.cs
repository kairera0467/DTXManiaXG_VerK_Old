	...
	internal class CAct演奏DrumsチップファイアD : CActivity
	{
		...
		public void Start( Eレーン lane, bool bフィルイン, bool b大波, bool b細波 )
		{
				...
				for ( int i = 0; i < 2; i++ )
				{
					for( int j = 0; j < FIRE_MAX; j++ )
					{
						if( !this.st火花[ j ].b使用中 )
						{
                            ...
                            int f = CDTXMania.ConfigIni.nExplosionFrames;
                            this.st火花[j].ctフレーム = new CCounter(0, f, 15, CDTXMania.Timer );
                ...
                for (int i = 0; i < 16; i++)
                {
                    for (int j = 0; j < 0x100; j++)
                    {
                        if (!this.st青い星[j].b使用中)
                        {
                            ...
                            double num7 = 0.7 + ( 1 / 100.0); // 拡散の大きさ
                            ...
                            this.st青い星[j].fY = (((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + 350 + (((float)Math.Sin((double)this.st青い星[j].f半径)) * this.st青い星[j].f半径)) - 170f); //Y座標
				...
				for( int i = 0; i < 0x10; i++ )
				{
					for( int j = 0; j < STAR_MAX; j++ )
					{
						if( !this.st青い星[ j ].b使用中 )
						{
                            ...
                            this.st青い星[ j ].fY = (((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + 350 + (((float)Math.Sin((double)this.st青い星[j].f半径)) * this.st青い星[j].f半径)) - 170f);
							this.st青い星[ j ].f加速度X = (float) ( num7 * Math.Cos( ( Math.PI * 2 * n回転初期値 ) / 360.0 ) );
							this.st青い星[ j ].f加速度Y = (float) ( num7 * ( Math.Sin( ( Math.PI * 2 * n回転初期値 ) / 360.0 ) - 0.2 ) );
							this.st青い星[ j ].f加速度の加速度X = 0.995f;
							this.st青い星[ j ].f加速度の加速度Y = 0.995f;
            ...
            if ((this.tx青い星 != null))
            {
                for (int i = 0; i < 1; i++)
                {
                    for (int j = 0; j < 50; j++)
                    {
                        if (!this.stエフェクト[j].b使用中)
                        {
                            this.stエフェクト[j].b使用中 = true;
                            int n回転初期値 = CDTXMania.Random.Next(0);
                            double num7 = 0.9 + (((double)CDTXMania.Random.Next(40)) / 100.0);
                            double num7MIR = 0.9 - (((double)CDTXMania.Random.Next(40)) / 100.0);
                            this.stエフェクト[j].nLane = (int)lane;
                            this.stエフェクト[j].ct進行 = new CCounter(0, 100, 7, CDTXMania.Timer);
                            this.stエフェクト[j].ct進行2 = new CCounter(-10, 10, 1, CDTXMania.Timer);
                            this.stエフェクト[j].fX = this.nレーンの中央X座標[(int)lane] + 320;
                            if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.A)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    this.stエフェクト[j].fX = this.nレーンの中央X座標_改[(int)lane] + 320;
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.B)
                            {
                                this.stエフェクト[j].fX = this.nレーンの中央X座標B[(int)lane] + 320;

                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    this.stエフェクト[j].fX = this.nレーンの中央X座標B_改[(int)lane] + 320;
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.C)
                            {
                                this.stエフェクト[j].fX = this.nレーンの中央X座標C[(int)lane] + 320;
                            }
                            this.stエフェクト[j].fY = (((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + 280 - (this.stエフェクト[i].ct進行2.n現在の値 * this.stエフェクト[i].ct進行2.n現在の値)));
                            this.stエフェクト[j].f加速度X = (float)(num7);
                            this.stエフェクト[j].f加速度XMIR = (float)(num7MIR);
                            this.stエフェクト[j].f加速度Y = (float)(num7 * (Math.Sin((Math.PI * 2 * n回転初期値) / 360.0) - 0.2));
                            this.stエフェクト[j].f加速度の加速度X = 0.995f;
                            this.stエフェクト[j].f加速度の加速度Y = 1.010f;
                            this.stエフェクト[j].f重力加速度 = 0.005f;
                            this.stエフェクト[j].f半径 = (float)(0.5 + (((double)CDTXMania.Random.Next(30)) / 100.0));
                            break;
                        }
                    }
                }
		...
		public override void OnManagedリソースの解放()
		{
			if( !base.b活性化してない )
			{
                ...
                CDTXMania.tテクスチャの解放(ref this.tx火花[5]);
                CDTXMania.tテクスチャの解放(ref this.tx火花[6]);
                CDTXMania.tテクスチャの解放(ref this.tx火花[7]);
                CDTXMania.tテクスチャの解放(ref this.tx火花[8]);
                CDTXMania.tテクスチャの解放(ref this.tx火花[9]);
                for (int i = 0; i < 10; i++)
                {
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
                ...
                for (int i = 0; i < 10; i++)
                {
                    if (this.stエフェクト[i].b使用中)
                    {
                        this.stエフェクト[i].n前回のValue = this.stエフェクト[i].ct進行.n現在の値;
                        this.stエフェクト[i].ct進行.t進行();
                        if (this.stエフェクト[i].ct進行.b終了値に達した)
                        {
                            this.stエフェクト[i].ct進行.t停止();
                            this.stエフェクト[i].b使用中 = false;
                        }
                        for (int n = this.stエフェクト[i].n前回のValue; n < this.stエフェクト[i].ct進行.n現在の値; n++)
                        {
                            this.stエフェクト[i].fX += this.stエフェクト[i].f加速度X;
                            this.stエフェクト[i].fY -= this.stエフェクト[i].f加速度Y;
                            this.stエフェクト[i].f加速度X *= this.stエフェクト[i].f加速度の加速度X;
                            this.stエフェクト[i].f加速度Y *= this.stエフェクト[i].f加速度の加速度Y;
                            this.stエフェクト[i].f加速度Y -= this.stエフェクト[i].f重力加速度;
                        }
                        Matrix mat2 = Matrix.Identity;
                        mat2 *= Matrix.Translation(this.stエフェクト[i].fX - SampleFramework.GameWindowSize.Width / 2, -(this.stエフェクト[i].fY - SampleFramework.GameWindowSize.Height / 2), 0f);
                        this.txNotes[stエフェクト[i].nLane].t3D描画(CDTXMania.app.Device, mat2, new Rectangle(this.nノーツの左上X座標[this.stエフェクト[i].nLane] + (this.nノーツの幅[this.stエフェクト[i].nLane] / 2), 0, this.nノーツの幅[this.stエフェクト[i].nLane] / 2, 10));
                    }
                }
                ...
                for (int i = 0; i < 10; i++)
                {
                    if (this.stエフェクト[i].b使用中)
                    {
                        ...
                        for (int n = this.stエフェクト[i].n前回のValue; n < this.stエフェクト[i].ct進行.n現在の値; n++)
                        {
                            this.stエフェクト[i].fX += this.stエフェクト[i].f加速度XMIR;
                            this.stエフェクト[i].fY -= this.stエフェクト[i].f加速度Y;
                            this.stエフェクト[i].f加速度X *= this.stエフェクト[i].f加速度の加速度X;
                            this.stエフェクト[i].f加速度Y *= this.stエフェクト[i].f加速度の加速度Y;
                            this.stエフェクト[i].f加速度Y -= this.stエフェクト[i].f重力加速度;
					...
					if( this.st火花[ i ].b使用中 )
					{
                        ...
                        float num5 = ((float)(0.8 * Math.Sin(num2 * 1.5707963267948966))) * this.st火花[i].fサイズ;
                            ...
                            if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.A)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    identity *= Matrix.Translation((this.nレーンの中央X座標_改[this.st火花[i].nLane] + (((float)Math.Cos((double)num3)) * num5)) - 320f, -(((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + (((float)Math.Sin((double)num3)) * num5)) - 170f), 0f);
                                }
                                else
                                {
                                    identity *= Matrix.Translation((this.nレーンの中央X座標[this.st火花[i].nLane] + (((float)Math.Cos((double)num3)) * num5)) - 320f, -(((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + (((float)Math.Sin((double)num3)) * num5)) - 170f), 0f);
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.B)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    identity *= Matrix.Translation((this.nレーンの中央X座標B_改[this.st火花[i].nLane] + (((float)Math.Cos((double)num3)) * num5)) - 320f, -(((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + (((float)Math.Sin((double)num3)) * num5)) - 170f), 0f);
                                }
                                else
                                {
                                    identity *= Matrix.Translation((this.nレーンの中央X座標B[this.st火花[i].nLane] + (((float)Math.Cos((double)num3)) * num5)) - 320f, -(((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + (((float)Math.Sin((double)num3)) * num5)) - 170f), 0f);
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.C)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    identity *= Matrix.Translation((this.nレーンの中央X座標C_改[this.st火花[i].nLane] + (((float)Math.Cos((double)num3)) * num5)) - 320f, -(((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + (((float)Math.Sin((double)num3)) * num5)) - 170f), 0f);
                                }
                                else
                                {
                                    identity *= Matrix.Translation((this.nレーンの中央X座標C[this.st火花[i].nLane] + (((float)Math.Cos((double)num3)) * num5)) - 320f, -(((CDTXMania.ConfigIni.bReverse.Drums ? 55f : ((float)this.iPosY)) + (((float)Math.Sin((double)num3)) * num5)) - 170f), 0f);
                                }
                            }
					...
					if( this.st大波[ i ].b使用中 )
					{
						...
						if( this.st大波[ i ].ct進行.n現在の値 >= 0 )
						{
                            ...
                            if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.A)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    matrix3 *= Matrix.Translation(this.nレーンの中央X座標_改[this.st大波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                                else
                                {
                                    matrix3 *= Matrix.Translation(this.nレーンの中央X座標[this.st大波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.B)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    matrix3 *= Matrix.Translation(this.nレーンの中央X座標B_改[this.st大波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                                else
                                {
                                    matrix3 *= Matrix.Translation(this.nレーンの中央X座標B[this.st大波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.C)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                {
                                    matrix3 *= Matrix.Translation(this.nレーンの中央X座標C_改[this.st大波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                                else
                                {
                                    matrix3 *= Matrix.Translation(this.nレーンの中央X座標C[this.st大波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                            }
					...
					if( this.st細波[ i ].b使用中 )
					{
						...
						if( this.st細波[ i ].ct進行.n現在の値 >= 0 )
						{
                            ...
                            if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.A)
                            {
                                ...
                                {
                                    matrix4 *= Matrix.Translation(this.nレーンの中央X座標[this.st細波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                                else
                                {
                                    matrix4 *= Matrix.Translation(this.nレーンの中央X座標_改[this.st細波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.B)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RCRD)
                                {
                                    matrix4 *= Matrix.Translation(this.nレーンの中央X座標B[this.st細波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                                else
                                {
                                    matrix4 *= Matrix.Translation(this.nレーンの中央X座標B_改[this.st細波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.C)
                            {
                                if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RCRD)
                                {
                                    matrix4 *= Matrix.Translation(this.nレーンの中央X座標C[this.st細波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                                else
                                {
                                    matrix4 *= Matrix.Translation(this.nレーンの中央X座標C_改[this.st細波[i].nLane] + 280 - SampleFramework.GameWindowSize.Width / 2, -((CDTXMania.ConfigIni.bReverse.Drums ? 55f : iPosY + 200) - SampleFramework.GameWindowSize.Height / 2), 0f);
                                }
                            }
        ...
        private readonly int[] nレーンの中央X座標B_改 = new int[] { 7, 71, 124, 240, 297, 349, 398, 500, 180, 180, 180 };
        ...
        private STエフェクト[] stエフェクト = new STエフェクト[20];
