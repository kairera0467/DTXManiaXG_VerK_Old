		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
                ...
                else
                {
                    ...
                    this.txLine = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\7_Dummy.png"));
                }
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    int x2 = (CDTXMania.stage演奏ドラム画面.actPad.st基本位置[index].x + (CDTXMania.ConfigIni.bReverse.Drums ? -10 : 32)) + CDTXMania.stage演奏ドラム画面.actPad.stパッド状態[index].nX座標オフセットdot;
                    int x3 = (CDTXMania.stage演奏ドラム画面.actPad.st基本位置[index].x + (CDTXMania.ConfigIni.bReverse.Drums ? -10 : 79)) - CDTXMania.stage演奏ドラム画面.actPad.stパッド状態[index].nX座標オフセットdotFLIP;
                    ...
                    if (index == 0) //LC
                    {
                        //this.txLC.t2D描画(CDTXMania.app.Device, x2, 0);
                        this.txLine.t2D描画(CDTXMania.app.Device, x2, 0, new Rectangle(0, 0, 70, 720));
                    }
                    ...
                    if (CDTXMania.ConfigIni.eDark == Eダークモード.OFF)
                    {
                        if (index == 1) //HH
                        {
                            this.txLC.t2D描画(CDTXMania.app.Device, x2 - 2, 0);
                            this.txLine.t2D描画(CDTXMania.app.Device, 365, 0, new Rectangle(70, 0, 2, 720));
                        }
                        if (index == 2) //SD
                        {
                        ...
                        if (index == 3) //BD
                        {
                            if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.A)
                            {
                                this.txLC.t2D描画(CDTXMania.app.Device, x2 - 18, 0);
                                this.txLC.t2D描画(CDTXMania.app.Device, x3 - 2, 0);
                            ...
                            {
                                this.txLC.t2D描画(CDTXMania.app.Device, x2 - 69, 0);
                                this.txLC.t2D描画(CDTXMania.app.Device, x3 - 51, 0);
                        ...
                        if (index == 4) //HT
                        {
                            ...
                            {
                                this.txLC.t2D描画(CDTXMania.app.Device, x2 - 18, 0);
                                this.txLC.t2D描画(CDTXMania.app.Device, x3 - 14, 0);
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.B)
                            {
                        ...
                        if (index == 7) //CY
                        {
                            this.txLine.t2D描画(CDTXMania.app.Device, 743, 0, new Rectangle(448, 0, 2, 720));
                            if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RCRD)
                                this.txLine.t2D描画(CDTXMania.app.Device, x3 - 69, 0, new Rectangle(450, 0, 70, 720));
                            else if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                this.txLine.t2D描画(CDTXMania.app.Device, x3 - 30, 0, new Rectangle(450, 0, 70, 720));
                        }
                        if (index == 8) //RD
                        {
                            if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RCRD)
                                this.txLine.t2D描画(CDTXMania.app.Device, x3 - 55, 0, new Rectangle(520, 0, 38, 720));
                            else if (CDTXMania.ConfigIni.eRDPosition == ERDPosition.RDRC)
                                this.txLine.t2D描画(CDTXMania.app.Device, x3 - 122, 0, new Rectangle(520, 0, 38, 720));
                        ...
                        if (index == 9) //LP
                        {
                            if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.A)
                            {
                                this.txLC.t2D描画(CDTXMania.app.Device, x2 - 12, 0);
                            }
                            else if (CDTXMania.ConfigIni.eLaneType.Drums == Eタイプ.B)
                            {

                                this.txLC.t2D描画(CDTXMania.app.Device, x2 + 45, 0);
                            }
                        }
                    }
                    #endregion
                        ...
                        for ( int k = 0; k < 3; k++ )
						{
							if( CDTXMania.ConfigIni.bReverse.Drums )
							{
								...
								for( int m = 0; m < w; m += 42 )
								{
									...
									{
										this.txFlush[ j + 8 ].t2D描画( CDTXMania.app.Device, x + m, y, new Rectangle( ( k * 42 ) + 2, 0, ( ( w - m ) < 0x2a ) ? ( w - m ) : 0x2a, 128 ) );
									}
								}
							}
							else
