...
namespace DTXMania
{
	internal class CAct演奏判定文字列共通 : CActivity
	{
		...
		protected struct STSTATUS
		{
            ...
            public CCounter ctGREAT;
            public CCounter ctGOOD;
            public CCounter ctPOOR;
            public CCounter ctMISS;
		}
		...
		{       
...
#endif
                new Rectangle( 0, 0,    225, 135 ),		// Perfect
				new Rectangle( 225, 0,  225, 135 ),		// Great
				new Rectangle( 450, 0,    225, 105 ),		// Good
				new Rectangle( 0, 0,    225, 90  ),		// Poor
				new Rectangle( 225, 0,  225, 90  ),		// Miss
				new Rectangle( 0, 0,    0,   0   ),		// Bad
				new Rectangle( 0, 0,    225, 135 )		// Auto
			};
			...
			this.stLag数値 = new STlag数値[ 0x18 ];		// #25370 2011.2.1 yyagi
			for ( int i = 0; i < 12; i++ )
			{
		...
		public virtual void Start( int nLane, E判定 judge, int lag )
		{
			...
			if( ( ( nLane >= 10 ) || ( ( (E判定文字表示位置) CDTXMania.ConfigIni.判定文字表示位置.Drums ) != E判定文字表示位置.表示OFF ) ) && ( ( ( nLane != 10 ) || ( ( (E判定文字表示位置) CDTXMania.ConfigIni.判定文字表示位置.Guitar ) != E判定文字表示位置.表示OFF ) ) && ( ( nLane != 11 ) || ( ( (E判定文字表示位置) CDTXMania.ConfigIni.判定文字表示位置.Bass ) != E判定文字表示位置.表示OFF ) ) ) )
			{
				this.st状態[ nLane ].ct進行 = new CCounter( 0, 220, 1, CDTXMania.Timer );
                this.st状態[ nLane ].ctPERFECT = new CCounter(0, 11, 15, CDTXMania.Timer);
                this.st状態[ nLane ].ctGREAT = new CCounter(0, 11, 20, CDTXMania.Timer);
                this.st状態[ nLane ].ctGOOD = new CCounter(0, 4, 50, CDTXMania.Timer);
                this.st状態[ nLane ].ctPOOR = new CCounter(0, 5, 50, CDTXMania.Timer);
                this.st状態[ nLane ].ctMISS = new CCounter(0, 5, 50, CDTXMania.Timer); 
		...
		public override void On活性化()
		{
			for( int i = 0; i < 12; i++ )
			{
                ...
                this.st状態[i].ctPERFECT = new CCounter();
                this.st状態[i].ctGREAT = new CCounter();
                this.st状態[i].ctGOOD = new CCounter();
                this.st状態[i].ctMISS = new CCounter();
                this.st状態[i].ctPOOR = new CCounter();
			}
			base.On活性化();
		...
		public override void On非活性化()
		{
			for( int i = 0; i < 12; i++ )
			{
                ...
                this.st状態[i].ctPERFECT = null;
                this.st状態[i].ctGREAT = null;
                this.st状態[i].ctGOOD = null;
                this.st状態[i].ct進行 = null;
                this.st状態[i].ct進行 = null;
			}
			base.On非活性化();
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
                ...
                this.tx判定文字列[0] = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\7_judge strings 1.png"));
                this.tx判定文字列[1] = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\7_judge strings 2.png"));
                this.tx判定文字列[2] = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\7_judge strings 3.png"));
				this.txlag数値 = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\7_level numbers.png" ) );
