...
//			this.On非活性化();
		...
		public override void OnManagedリソースの作成()
		{
            ...
            for (int i = 0; i < 13; i++ )
                this.iベースパネル[i] = Image.FromFile(CSkin.Path(@"Graphics\5_Panel.png"));
            this.txパネル本体 = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\5_preimage panel.png"), false);
            this.tx曲名バー.Score = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\5_preimage default.png"), false);
			this.tx曲名バー.Box = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\5_num.png" ), false );
			this.tx曲名バー.Other = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\5_num.png" ), false );
			this.tx選曲バー.Score = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\5_num.png" ), false );
			this.tx選曲バー.Box = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\5_num.png" ), false );
			this.tx選曲バー.Other = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\5_num.png" ), false );
			...
			try
			{
				using ( Bitmap image = new Bitmap( 340, 200 ) )
				using ( Graphics graphics = Graphics.FromImage( image ) )
				{
		...
		public override void OnManagedリソースの解放()
		{
            ...
            for (int i = 0; i < 13; i++)
            {
                ...
                CDTXMania.t安全にDisposeする(ref this.stバー情報[i].tx選択曲以外のタイトル名L);
                CDTXMania.t安全にDisposeする(ref this.stバー情報[i].tx選択曲以外のタイトル名R);
                this.iベースパネル[i].Dispose();
                this.iバー背景.Dispose();
            }
            ...
            CDTXMania.tテクスチャの解放( ref this.txバー背景L );
            CDTXMania.tテクスチャの解放( ref this.txバー背景R);
		...
		public override int On進行描画()
		{
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
					if( this.ct登場アニメ用[ i ].n現在の値 >= 0 )
					{
						...
						if( i == 5 )
						{
							...
							int x = 552 - ( (int) ( width * db回転率 ) );
							int y = 300;
							this.tバーの描画( 432, 0x10f, this.stバー情報[ nパネル番号 ].eバー種別, true, i );
							//-----------------
							#endregion
							#region [ タイトル名テクスチャを描画。]
							//-----------------
							if( this.stバー情報[ nパネル番号 ].txタイトル名 != null )
								this.stバー情報[ nパネル番号 ].txタイトル名.t2D描画( CDTXMania.app.Device, 552 , 300 );
							//-----------------
							#endregion
							#region [ スキル値を描画。]
							//-----------------
							if( ( this.stバー情報[ nパネル番号 ].eバー種別 == Eバー種別.Score ) && ( this.e楽器パート != E楽器パート.UNKNOWN ) )
								this.tスキル値の描画( 500, 312 , this.stバー情報[ nパネル番号 ].nスキル値[ (int) this.e楽器パート ] );
							//-----------------
							#endregion
						}
						else
						{
							...
							this.tバーの描画( x, y, this.stバー情報[ nパネル番号 ].eバー種別, false, i );
							//-----------------
							#endregion
							#region [ タイトル名テクスチャを描画。]
							//-----------------
							...
							//-----------------
							if( ( this.stバー情報[ nパネル番号 ].eバー種別 == Eバー種別.Score ) && ( this.e楽器パート != E楽器パート.UNKNOWN ) )
								this.tスキル値の描画( x + 34, y + 0x10, this.stバー情報[ nパネル番号 ].nスキル値[ (int) this.e楽器パート ] );
							//-----------------
							#endregion
						}
					}
			...
			else
			{
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
					...
					int x = this.pt曲名の基本座標[ n見た目の行番号 ].X + ( (int) ( ( this.pt曲名の基本座標[ n次のパネル番号 ].X - this.pt曲名の基本座標[ n見た目の行番号 ].X ) * ( ( (double) Math.Abs( this.n現在のスクロールカウンタ ) ) / 100.0 ) ) );
					int y = this.pt曲名の基本座標[ n見た目の行番号 ].Y + ( (int) ( ( this.pt曲名の基本座標[ n次のパネル番号 ].Y - this.pt曲名の基本座標[ n見た目の行番号 ].Y ) * ( ( (double) Math.Abs( this.n現在のスクロールカウンタ ) ) / 100.0 ) ) );
                    int x2 = this.ptバーの基本座標[n見た目の行番号].X + ((int)((this.ptバーの基本座標[n次のパネル番号].X - this.ptバーの基本座標[n見た目の行番号].X) * (((double)Math.Abs(this.n現在のスクロールカウンタ)) / 100.0)));
                    int y2 = this.ptバーの基本座標[n見た目の行番号].Y + ((int)((this.ptバーの基本座標[n次のパネル番号].Y - this.ptバーの基本座標[n見た目の行番号].Y) * (((double)Math.Abs(this.n現在のスクロールカウンタ)) / 100.0)));
					...
					if( ( i == 5 ) && ( this.n現在のスクロールカウンタ == 0 ) )
					{
						...
						if( this.stバー情報[ nパネル番号 ].txタイトル名 != null )
							this.stバー情報[ nパネル番号 ].txタイトル名.t2D描画( CDTXMania.app.Device, 554, 204 );
						//-----------------
						#endregion
					}
					else
					{
						...
						#region [ バーテクスチャを描画。]
						//-----------------
						this.tバーの描画( x2, y2, this.stバー情報[ nパネル番号 ].eバー種別, false, i );
						//-----------------
						#endregion
						#region [ タイトル名テクスチャを描画。]
						//-----------------
                        if (this.stバー情報[nパネル番号].tx選択曲以外のタイトル名L != null)
                        {
                            if(i <= 5)
                                this.stバー情報[nパネル番号].tx選択曲以外のタイトル名L.t2D描画(CDTXMania.app.Device, x, y);
                            else if(i >= 6)
                                this.stバー情報[nパネル番号].tx選択曲以外のタイトル名R.t2D描画(CDTXMania.app.Device, x, y);
                        }
						//-----------------
						#endregion
					}
        ...
        private readonly Point[] ptバーの基本座標 = new Point[] { new Point(-1456, 198), new Point(-1280, 212), new Point(-1080, 228), new Point(-870, 248), new Point(-554, 274), new Point(-380, 260), new Point(360, 0), new Point(780, 260), new Point(1032, 238), new Point(1250, 220), new Point(1480, 200), new Point(1800, 210), new Point(-1600, 188), new Point(-1600, 188) };
        private readonly Point[] pt曲名の基本座標 = new Point[] { new Point(-1456, 198), new Point(-1280, 212), new Point(-1080, 228), new Point(-870, 248), new Point(-554, 274), new Point(-380, 260), new Point(410, 290), new Point(780, 260), new Point(1032, 238), new Point(1250, 220), new Point(1480, 200), new Point(1800, 210), new Point(-1600, 188),  new Point(-1600, 188)};
        ...
        private Image iバー背景;
        private Image[] iベースパネル = new Image[13];
		private STバー tx曲名バー;
		...
		{
            //ここで選曲画面のタイトル、スキルを読み込んでいる。及び画面に反映させている。
            //そこを考えると、stバー情報[i]などにジャケット読み込み等の処理を追加する。
			C曲リストノード song = this.r現在選択中の曲;
		...
		private void tバーの描画( int x, int y, Eバー種別 type, bool b選択曲,int n現在の位置 )
		{
