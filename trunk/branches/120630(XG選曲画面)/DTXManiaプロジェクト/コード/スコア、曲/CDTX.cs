		...
		public class CChip : IComparable<CDTX.CChip>
		{
			...
			public override string ToString()
			{
				string[] chToStr = 
				{
					...
					"??", "??", "??", "??", "??", "??", "??", "??", 
					"??", "??", "??", "??", "??", "??", "??", "??", 
					"小節線", "拍線", "MIDIコーラス", "フィルイン", "AVI", "AVI2", "BMPレイヤ4", "BMPレイヤ5",
					"BMPレイヤ6", "BMPレイヤ7", "??", "??", "??", "??", "??", "??", 
...
//		public bool bBASS演奏で直前のBASSを消音する;
        ...
        private static void tミラーチップのチャンネルを指定する(CChip chip, int nミラー化前チャンネル番号)
        {
            switch (nミラー化前チャンネル番号)
            {
                ...
                case 0x12:
                    chip.nチャンネル番号 = (CDTXMania.ConfigIni.eNumOfLanes.Drums == Eタイプ.C) ? 0x15 : 0x17;
                    return;
                ...
                case 0x15:
                    chip.nチャンネル番号 = (CDTXMania.ConfigIni.eNumOfLanes.Drums == Eタイプ.C) ? 0x12 : 20;
                    return;
		...
		private bool t入力・行解析・チップ配置( string strコマンド, string strパラメータ, string strコメント )
		{
			...
			for( int i = 0; i < ( n文字数 / 2 ); i++ )	// 2文字で1オブジェクト数値
			{
				...
				if( nチャンネル番号 == 0x53 )
				{
					...
					{
						chip.n発声位置 += 1;	// 同じく、フィルインOFFチップは12分音符ほど後ろへ移動。
					}
				}
				//-----------------
				#endregion
