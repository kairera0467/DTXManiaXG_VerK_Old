				...
				for( int i = 0; i < nChips; i++ )
				{
                    ...
                    if (nChipNo != 0)
                    {
                        ...
                        switch ( nCh )
                        {
                            ...
                            case 152:
                                item = new Cチップ();
                                item.nレーン番号0to = nレーン番号0to2;
                                item.n位置grid = i;
                                item.n値・整数1to1295 = 1;
                                item.n読み込み時の解像度 = nChips;
                                c小節.listチップ.Add(item);
                                item = new item();
                                item.nレーン番号0to = nレーン番号0to4;
                                item.n位置grid = i;
                                item.n値・整数1to1295 = 1;
                                item.n読み込み時の解像度 = nChips;
                                c小節.listチップ.Add(item);
                                item = new Cチップ();
                                item.nレーン番号0to = nレーン番号0to5;
                                item.n位置grid = i;
                                item.n値・整数1to1295 = 1;
                                item.n読み込み時の解像度 = nChips;
                                c小節.listチップ.Add(item);
