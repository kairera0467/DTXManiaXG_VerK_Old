...
//		Debug.WriteLine( "★Queue: remove " + Path.GetFileName( stm.csound.strファイル名 ));
        ...
        protected E判定 tチップのヒット処理( long nHitTime, CDTX.CChip pChip, E楽器パート screenmode, bool bCorrectLane )
        {
            ...
            if (CDTXMania.ConfigIni.nSkillMode == 1)
            {
                ...
                {
                    if ((!bPChipIsAutoPlay && (pChip.e楽器パート == E楽器パート.DRUMS)) && (eJudgeResult != E判定.Miss) && (eJudgeResult != E判定.Bad))
                    {
                        #region[ドラム]
                        int nCombos = this.actCombo.n現在のコンボ数.Drums;
                        float nScoreDelta = 0;
        ...
        protected void tキー入力()
        {
            ...
            if (((base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED)) && (base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED_フェードアウト))
            {
                ...
                {	// alt + UpArrow (CommonBGMAdjust)
                    CDTXMania.DTX.t各自動再生音チップの再生時刻を変更する_共通((keyboard.bキーが押されている((int)SlimDX.DirectInput.Key.LeftControl) || keyboard.bキーが押されている((int)SlimDX.DirectInput.Key.RightControl)) ? 1 : 10 );
                    CDTXMania.DTX.tWave再生位置自動補正();
                }
...
//		protected abstract void t進行描画・AVI();
		...
		protected bool t進行描画・チップ( E楽器パート ePlayMode )
		{
			...
			for ( int nCurrentTopChip = this.n現在のトップChip; nCurrentTopChip < dTX.listChip.Count; nCurrentTopChip++ )
			{
                ...
                pChip.nバーから終点チップまでの距離dot.Guitar = (int)((pChip.nノーツ終了時刻ms - CSound管理.rc演奏用タイマ.n現在時刻) * ScrollSpeedGuitar);
                    ...
                    case 0x03:	// BPM変更
                        if (!pChip.bHit && (pChip.nバーからの距離dot.Drums < 0))
                        {
                            ...
                            if( CDTXMania.ConfigIni.bDrums有効 )
                            {
                                ...
                                this.actBPMBar.ctBPMバー = new CCounter(0, 16.0, CDTXMania.stage演奏ドラム画面.UnitTime, CSound管理.rc演奏用タイマ );
                            ...
                            else if( CDTXMania.ConfigIni.bGuitar有効 )
                            {
                                ...
                                this.actBPMBar.ctBPMバー = new CCounter(0.0, 16.0, CDTXMania.stage演奏ギター画面.UnitTime, CSound管理.rc演奏用タイマ );
                    ...
                    case 0x08:	// BPM変更(拡張)
                        if (!pChip.bHit && (pChip.nバーからの距離dot.Drums < 0))
                        {
                            ...
                            if (dTX.listBPM.ContainsKey(pChip.n整数値・内部番号))
                            {
                                ...
                                {
                                    CDTXMania.stage演奏ドラム画面.UnitTime = ((60.0 / (CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM) / 16.0 ));
                                    this.actBPMBar.ctBPMバー = new CCounter(1.0, 16.0, CDTXMania.stage演奏ドラム画面.UnitTime, CSound管理.rc演奏用タイマ );
                                    CDTXMania.stage演奏ドラム画面.ctコンボ動作タイマ = new CCounter(1.0, 16.0, ((60.0 / (CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM) / 16.0)), CSound管理.rc演奏用タイマ);
                                }
                                else if( CDTXMania.ConfigIni.bGuitar有効 )
                                {
                                    CDTXMania.stage演奏ギター画面.UnitTime = ((60.0 / (CDTXMania.stage演奏ギター画面.actPlayInfo.dbBPM) / 16.0 ));
                                    this.actBPMBar.ctBPMバー = new CCounter(1.0, 16.0, CDTXMania.stage演奏ギター画面.UnitTime, CSound管理.rc演奏用タイマ );
                                    CDTXMania.stage演奏ギター画面.ctコンボ動作タイマ = new CCounter(1.0, 16.0, ((60.0 / (CDTXMania.stage演奏ギター画面.actPlayInfo.dbBPM) / 16.0)), CSound管理.rc演奏用タイマ);
                                }
                            }
                        }
...
//					nCurrentTopChip = ++this.n現在のトップChip;
        ...
        public bool bCheckAutoPlay(CDTX.CChip pChip)
        {
            ...
            CDTXMania.DTX.tギターレーンのチップパターンを取得する( pChip.nチャンネル番号, ref bGtBsR, ref bGtBsG, ref bGtBsB, ref bGtBsY, ref bGtBsP );
            
            //28 A8
            if( pChip.nチャンネル番号 == 0x28 || pChip.nチャンネル番号 == 0xA8 )
                bGtBsW = true;
            else if( pChip.nチャンネル番号 == 0x20 || pChip.nチャンネル番号 == 0xA0 )
                bGtBsO = true;
			...
			int chipWidth, int guitarNormalX, int guitarLeftyX, int bassNormalX, int bassLeftyX, int drawDeltaX, int chipTexDeltaX )
		{
			...
			if ( configIni.bGuitar有効 )
			{
                ...
                CDTXMania.DTX.tギターレーンのチップパターンを取得する( pChip.nチャンネル番号, ref bChipHasR, ref bChipHasG, ref bChipHasB, ref bChipHasY, ref bChipHasP );
                if( pChip.nチャンネル番号 == 0x28 || pChip.nチャンネル番号 == 0xA8 )
                    bChipHasW = true;
                else if( pChip.nチャンネル番号 == 0x20 || pChip.nチャンネル番号 == 0xA0 )
                    bChipIsO = true;
				...
				if ( !pChip.bHit && pChip.b可視 )
				{
					...
					int yEnd = configIni.bReverse[ instIndex ] ? ( barYReverse - pChip.nバーから終点チップまでの距離dot[ instIndex ] ) : ( barYNormal + pChip.nバーから終点チップまでの距離dot[ instIndex ] );
...
//Trace.TraceInformation( "chip={0:x2}, E楽器パート={1}, x={2}", pChip.nチャンネル番号, inst, x );
							if ( bChipHasR )
							{
                                if (inst == E楽器パート.GUITAR)
                                {
                                    ...
                                    if( pChip.nノーツ終了位置 != 0 )
                                        this.txチップ.t2D描画(CDTXMania.app.Device, ( CDTXMania.ConfigIni.bLeft.Guitar ? 244 : 88 ), yEnd - chipHeight / 2, new Rectangle(0, 0, 38, 10));
                                }
							...
							if ( bChipHasG )
							{
                                if (inst == E楽器パート.GUITAR)
                                {
                                    ...
                                    if( pChip.nノーツ終了位置 != 0 )
                                        this.txチップ.t2D描画(CDTXMania.app.Device, ( CDTXMania.ConfigIni.bLeft.Guitar ? 205 : 127 ), yEnd - chipHeight / 2, new Rectangle(38, 0, 38, 10));
                                }
                                else if(inst == E楽器パート.BASS)
                                {
							...
							if ( bChipHasB )
							{
                                if (inst == E楽器パート.GUITAR)
                                {
                                    ...
                                    if( pChip.nノーツ終了位置 != 0 )
                                        this.txチップ.t2D描画(CDTXMania.app.Device, 166, yEnd - chipHeight / 2, new Rectangle(76, 0, 38, 10));
                                }
                                else if (inst == E楽器パート.BASS)
                                {
                            ...
                            if ( bChipHasY )
                            {
                                if (inst == E楽器パート.GUITAR)
                                {
                                    ...
                                    if( pChip.nノーツ終了位置 != 0 )
                                        this.txチップ.t2D描画(CDTXMania.app.Device, ( CDTXMania.ConfigIni.bLeft.Guitar ? 127 : 205 ), yEnd - chipHeight / 2, new Rectangle(114, 0, 38, 10));
                                }
                                else if (inst == E楽器パート.BASS)
                                {
                            ...
                            if ( bChipHasP )
                            {
                                if (inst == E楽器パート.GUITAR)
                                {
                                    ...
                                    if( pChip.nノーツ終了位置 != 0 )
                                        this.txチップ.t2D描画(CDTXMania.app.Device, ( CDTXMania.ConfigIni.bLeft.Guitar ? 88 : 244 ), yEnd - chipHeight / 2, new Rectangle(152, 0, 38, 10));
                                }
                                else if (inst == E楽器パート.BASS)
                                {
		...
		protected virtual void t入力処理・ギターベース(E楽器パート inst)
		{
            ...
            //CDTXMania.act文字コンソール.tPrint( 0, 0, C文字コンソール.Eフォント種別.白, chip.nチャンネル番号.ToString() );
            if( chip != null )
            {
