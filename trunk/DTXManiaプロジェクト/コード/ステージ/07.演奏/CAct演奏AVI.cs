        ...
        public unsafe int t進行描画(int x, int y)
        {
            ...
            if ((!base.b活性化してない))
            {
                if( ( ( this.tx描画用 != null )) && ( this.dsBGV != null || this.rAVI != null ) ) //クリップ無し曲での進入防止。
                {
                    ...
                    //    this.n移動開始時刻ms = -1L;
                    //}
                    if((((this.n前回表示したフレーム番号 != frameNoFromTime) || !this.bフレームを作成した)) && (fClipアスペクト比 < 1.77f || CDTXMania.ConfigIni.bDirectShowMode == false))
                    {
                        //this.pBmp = this.rAVI.avi.tGetBitmap(frameNoFromTime);
                ...
                if ((((this.n総移動時間ms != -1L) && (((this.rAVI != null )) && (this.rAVI.avi != null)) || (this.dsBGV != null && this.dsBGV.dshow != null) )) && ( this.bWindowMode ) && this.tx描画用 != null && (CDTXMania.ConfigIni.bAVI有効))
                {
                    ...
                    if (CDTXMania.ConfigIni.bGraph有効 && CDTXMania.ConfigIni.bDrums有効)
                    {
                        ...
                        if( CDTXMania.ConfigIni.bDirectShowMode )
                        {
                            ...
                            else if( this.n総移動時間ms != -1 && this.fClipアスペクト比 < 0.96f )
                            {
                    ...
                    else if (!CDTXMania.ConfigIni.bGraph有効 && CDTXMania.ConfigIni.bDrums有効)
                    {
                        ...
                        if( CDTXMania.ConfigIni.bDirectShowMode )
                        {
                            ...
                            else if( this.n総移動時間ms != -1 && this.fClipアスペクト比 < 1.77f )
                            {
